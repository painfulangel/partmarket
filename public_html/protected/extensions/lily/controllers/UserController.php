...
class UserController extends Controller {
    ...
    public function accessRules() {
        return array(
            ...
            array('deny',
                'actions' => array('login', 'fastRegistration', 'onetime'),
                'users' => array('@'),
            ),
        );
    }
        ...
        if (!$model_new && $model->validate() && isset($model->service)) {
            ...
            if ($authIdentity->authenticate()) {
                if ($model->service == 'email' && $authIdentity->errorCode == LEmailService::ERROR_INFORMATION_MAIL_FAILED)
                    Yii::app()->user->setFlash('lily-login-error', Yii::t('lily', "Your account was created, but it failed to send you email with account information."));
                ...
                if ($identity->authenticate()) {
                    ...
                    if ($user->state == LUser::BANNED_STATE) {
                        Yii::app()->user->setFlash('lily-login-error', Yii::t('lily', "Your account was put under ban. Please contact to site administrator for details."));
                    } else {
                        $result = Yii::app()->user->login($identity, $model->rememberMe ? LilyModule::instance()->sessionTimeout : 0);
                        if ($result)
                            Yii::app()->user->setFlash('lily-login-success', Yii::t('lily', 'You were successfully logged in.'));
                        else
                            throw new LException("login() returned false");
                    }
                    
                    //!!! Recalculate cart prices for price group of user
                    Yii::app()->getModule('shop_cart')->recalculateCart();
            ...
            if ($model->service == 'email') {
                switch ($authIdentity->errorCode) {
                    case LEmailService::ERROR_ACTIVATION_MAIL_SENT:
                        Yii::app()->user->setFlash('lily-login-success', Yii::t('lily', "Activation e-mail sent."));
                        break;
                    case LEmailService::ERROR_ACTIVATION_MAIL_FAILED:
                        Yii::app()->user->setFlash('lily-login-error', Yii::t('lily', "Failed to send account activation email."));
                        break;
                    case LEmailService::ERROR_AUTH_FAILED:
                        Yii::app()->user->setFlash('lily-login-error', Yii::t('lily', "Failed to authenticate (email-password mismatch)."));
                        break;
                    case LEmailService::ERROR_NOT_REGISTERED:
                        Yii::app()->user->setFlash('lily-login-error', Yii::t('lily', "Account with given e-mail is not registered. You have to pass registration."));
                        $authIdentity->cancelUrl = $this->createUrl('register', array('email' => $authIdentity->email));
                        break;
                }
            } else {
                Yii::app()->user->setFlash('lily-login-error', Yii::t('lily', 'Failed to authenticate.'));
            }
            $authIdentity->cancel();
        }
    ...
    public function actionRegister() {
        $this->redirect(array('/userControl/adminUserProfile/createNewUser'));
        return;
    ...
    public function actionActivate($code) {
        ...
        switch ($errorCode) {
            case 0:
            case 3:
                $msg = Yii::t('lily', "Your account was successfully activated. Now you can login using your email.");
                break;
            case 1:
            case 2:
                $msg = Yii::t('lily', "Your code is wrong or have expired. Please request a new one.");
                break;
        }
        if (LilyModule::instance()->accountManager->sendMail) {
            if ($errorCode == 0) {
                $msg .= "<br />" . Yii::t('lily', "An email with account details was sent to your email.");
            } else if ($errorCode == 3) {
                $msg .= "<br/ >" . Yii::t('lily', "An email with account details sending failed. Please contact site administrator.");
            }
        }
    ...
    public function actionSwitch_state($mode = LUser::DELETED_STATE, $uid = null) {
        ...
        if (($mode == LUser::DELETED_STATE && ($user->state == LUser::DELETED_STATE || !Yii::app()->user->checkAccess('deleteUser', array('user' => $user)))) || ($mode == LUser::BANNED_STATE && ($user->state == LUser::BANNED_STATE || !Yii::app()->user->checkAccess('banUser', array('user' => $user)))) || ($mode == LUser::ACTIVE_STATE && ($user->state == LUser::ACTIVE_STATE || ($user->state == LUser::DELETED_STATE && !Yii::app()->user->checkAccess('restoreUser', array('user' => $user))) || ($user->state == LUser::BANNED_STATE && !Yii::app()->user->checkAccess('unbanUser', array('user' => $user)))
                )))
            throw new CHttpException(403);
        if ($approved) {
            $user->state = $mode;
            ...
            if ($mode < LUser::ACTIVE_STATE && $uid == Yii::app()->user->id) {
                ...
                switch ($mode) {
                    case LUser::ACTIVE_STATE: $msg = Yii::t('lily', 'User {user} was successfully activated', array('{user}' => $user->name));
                        break;
                    case LUser::DELETED_STATE: $msg = Yii::t('lily', 'User {user} was successfully deleted', array('{user}' => $user->name));
                        break;
                    case LUser::BANNED_STATE: $msg = Yii::t('lily', 'User {user} was successfully banned', array('{user}' => $user->name));
                        break;
                }
    ...
    public function actionOnetime($token, $redirectUrl = null) {
        ...
        if ($result)
            Yii::app()->user->setFlash('lily.onetime.success', Yii::t('lily', 'You were successfully logged in.'));
        else
            Yii::app()->setFlash('lily.onetime.error', Yii::t('lily', 'Wrong one-time login token.'));
    ...
    public function actionInit($action) {
        if (!LilyModule::instance()->userIniter->isStarted)
            throw new CHttpException(403);
        //echo LilyModule::instance()->userIniter->stepId;
        //echo $action;
        //die;
        if (($action == 'start' && LilyModule::instance()->userIniter->stepId == 0) || ($action == 'finish' && LilyModule::instance()->userIniter->stepId == LilyModule::instance()->userIniter->count - 1)) {
