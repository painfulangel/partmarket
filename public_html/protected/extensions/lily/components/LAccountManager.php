...
class LAccountManager extends CApplicationComponent {
    ...
    public function sendInformationMail(LAccount $account) {
        ...
        if (!isset($subject) || !is_string($subject))
            $subject = Yii::t('lily', 'E-mail registration on {siteName}', array('{siteName}' => Yii::app()->config->get('Site.SiteName')));
        ...
        if (isset($this->informationMailView))
            ...
            $message->setBody(Yii::t('lily', 'Your email was successfully registered on <a href="{siteUrl}">{siteName}</a>!', array('{siteUrl}' => Yii::app()->createAbsoluteUrl(Yii::app()->homeUrl), '{siteName}' => Yii::app()->config->get('Site.SiteName'))), 'text/html');
        $message->addTo($account->id);
        $message->from = Yii::app()->config->get('Site.NoreplyEmail');//$this->adminEmail;
    ...
    public function sendActivationMail(LEmailAccountActivation $code, LUser $user = null) {
        ...
        if (isset($this->activationMailView))
            ...
            $message->setBody(Yii::t('lily', 'Your email was used in registration on <a href="{siteUrl}">{siteName}</a>.<br />
...
administration of {siteName}.', array('{siteUrl}' => Yii::app()->createAbsoluteUrl(Yii::app()->homeUrl), '{siteName}' => Yii::app()->config->get('Site.SiteName'),
    ...
    public function sendRestoreMail(LAccount $account) {
        ...
        if (!isset($subject) || !is_string($subject))
            $subject = Yii::t('lily', 'Password restoration on {siteName}', array('{siteName}' => Yii::app()->config->get('Site.SiteName')));
        $message->setSubject($subject);
        $message->view = $this->restoreMailView;
        if (isset($this->restoreMailView))
            $message->setBody(array('account' => $account), 'text/html');
        else
            $message->setBody(Yii::t('lily', 'Your email is registered on <a href="{siteUrl}">{siteName}</a>.<br />
