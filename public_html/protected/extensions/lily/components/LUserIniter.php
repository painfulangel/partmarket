...
class LUserIniter extends CApplicationComponent {
    ...
    public function start() {
        ...
        if (!in_array($route, $this->step->allowed) && !in_array($route, LilyModule::instance()->allowedRoutes) && !in_array($route, array(LilyModule::route('user/logout')))
        ) {
...
//        $this->finish();
//        return;
        //LilyModule::instance()->session->data->userInitData->stepId+=10;
        
        LilyModule::instance()->user->inited = true;
//        print_r(LilyModule::instance());
        if (!LilyModule::instance()->user->save())
            throw new CDbException("can't save user");
    }
