...
class LEmailService extends EAuthServiceBase implements IAuthService {
     ...
     */
    public function authenticate($forceRegisterEmail = false, $checkPassword = true, $forse = false) {
        $email = $this->email;
        ...
        if (!isset($account)) {
            if (LilyModule::instance()->accountManager->registerEmail || $forceRegisterEmail) {
                ...
                if (!$forse)
                    ...
                    LilyModule::instance()->accountManager->performRegistration($email, $password, false, null, $this->user, $this->rememberMe);
                if (LilyModule::instance()->accountManager->activate && !$forse) {
