...
class EIMap extends CComponent {
    ...
    public function getAttachments($msgId, $structure = null, $save = true, $return = false) {
        ...
        for ($i = 0, $idx = 0, $len = count($parts); $i < $len; $i++) {
            $part = $parts[$i];
//            print_r($part);
            if (($structure->ifdisposition && isset($part->disposition) && strtolower($part->disposition) == 'attachment' && $structure->ifdparameters) ||
                    ( in_array(strtolower($part->subtype), $imgTypes)) ||
                    (isset($part->disposition) && strtolower($part->disposition) == 'attachment' && in_array(strtolower($part->subtype), $fileTypes))
            ) {
...
//        print_r(iconv_mime_decode($part->parameters[0]->value, 0, "UTF-8"));
        $attachment['filename'] = (uniqid()) . '-' . iconv_mime_decode($part->parameters[0]->value, 0, "UTF-8");
