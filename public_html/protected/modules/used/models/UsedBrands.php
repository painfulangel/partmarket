...
/**
 * This is the model class for table "used_brands".
 *
 * The followings are the available columns in table 'used_brands':
 * @property integer $id
 ...
 * @property UsedMod[] $usedMods
 * @property UsedModels[] $usedModels
 */
class UsedBrands extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('name, title, image, slug', 'length', 'max'=>255),
			array('keywords, description, text', 'safe'),
			array('image', 'unsafe', 'on'=>'update'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, slug, title, keywords, description, text, image, sort, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'usedItems' => array(self::HAS_MANY, 'UsedItems', 'brand_id'),
			'usedMods' => array(self::HAS_MANY, 'UsedMod', 'brand_id'),
			'usedModels' => array(self::HAS_MANY, 'UsedModels', 'brand_id'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'status' => Yii::t(UsedModule::TRANSLATE_PATH, 'Status'),
		);
	}
	 ...
	 * @return UsedBrands the static model class
	 */
