...
/**
 * This is the model class for table "used_models".
 *
 * The followings are the available columns in table 'used_models':
 * @property integer $id
 ...
 */
class UsedModels extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('brand_id, sort', 'numerical', 'integerOnly'=>true),
			array('name, slug, title', 'length', 'max'=>255),
			array('keywords, description, text', 'safe'),
			array('image', 'unsafe', 'on'=>'update'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, brand_id, name, slug, title, keywords, description, text, image, sort', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'usedItems' => array(self::HAS_MANY, 'UsedItems', 'model_id'),
			'usedMods' => array(self::HAS_MANY, 'UsedMod', 'model_id'),
	 ...
	 * @return UsedModels the static model class
	 */
