...
//            '' => 'string',
    ...
    public function rules() {
        ...
        return array(
            ...
            array('text, detail_type, detail_subtype, short_text, detail_type_slug, detail_subtype_slug', 'safe'),
    ...
    public function checkuniqueslug($attribute) {
        ...
        $items = KatalogVavtoCathegorias::model()->findAllByAttributes(array('slug' => $this->slug));
        $count = count($items);
        for ($i = 0; $i < $count; $i ++) {
            if ($items[$i]->id != $this->id) $id = $items[$i]->id;
        }
        ...
        if ($id != null && $id != $this->id)
            ...
            $items = KatalogVavtoItems::model()->findAllByAttributes(array('slug' => $this->slug));
            $count = count($items);
            for ($i = 0; $i < $count; $i ++) {
                if ($items[$i]->id != $this->id) $id = $items[$i]->id;
            }
...
//                ),
    ...
    public function beforeSave() {
        if (parent::beforeSave()) {
            ...
            if (empty($this->load_lang)) {
                $this->detail_type_slug = CMyTranslit::getText(mb_strtolower($this->detail_type));
                $this->detail_subtype_slug = CMyTranslit::getText(mb_strtolower($this->detail_subtype));
            }
    ...
    public function attributeLabels() {
        return array(
            ...
            'detail_subtype' => Yii::t('katalogVavto', 'Items Subtype'),
...
//            $criteria->compare('text', $this->search_text, true, 'OR');
    ...
    public function getItemsDataProvider() {
        ...
        if (!empty($subtype))
            $criteria->compare('detail_subtype_slug', $subtype);

        $criteria->compare('active_state', 1);

        $criteria->select = 'image, short_text, id, title, sub_title, text, article, slug';
    ...
    public function importTXT($filename, $fileCharset, &$tmp_echo) {
        ...
        if ($n > 1) {
            ...
            for ($i = 1; $i < $n; $i++) {
                ...
                $model->sub_title = $string[4];

                $model->detail_type = $string[6];
                $model->detail_subtype = $string[7];

                $model->slug = $string[8];

                $model->meta_title = $string[9];
                $model->meta_description = $string[10];
                $model->meta_keywords = $string[11];

                if (array_key_exists(12, $string)) $model->active_state = $string[12];
                if (array_key_exists(13, $string)) $model->in_stock = $string[13];
                
                if (empty($model->meta_title))
                    $model->meta_title = $model->title;
                $temp_i = 1;
                if (empty($string[8]))
                    $string[8] = CMyTranslit::getText($model->title);
                ...
                if (empty($string[5])) {
                    ...
                    $temp = KatalogVavtoCars::model()->findByAttributes(array('title' => $string[5]));
                    if ($temp != NULL) {
                        $model->cathegory_id = $temp->id;
                    } else {
                        echo Yii::t('katalogVavto', 'No car')." $string[5] ".Yii::t('katalogVavto', 'in line')." $i";
                        echo '<br>';
						//flush();
                        continue;
                    }
    ...
    public function export() {
        ...
        $export = 'Id;'.
                  ...
                  Yii::t('katalogVavto', 'Items Subtype').';'.
        ...
        foreach ($data as $value) {
            $export .= $value['id'].';'.
            		   $value['article'].';'.
            		   $value['title'].';'.
                       str_replace(array('&nbsp;', ';'), ' ', nl2br($value['text'])).';'.
            		   $value['sub_title'].';'.
            		   $value['cathegory_title'].';'.

            		   $value['detail_type'].';'.
                       $value['detail_subtype'].';'.
