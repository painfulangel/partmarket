...
$(function() {
	var e = $('#qgTree').find('a.active');
	
	expandMe(e);
});

function expandMe(e) {
	if (e.parent().parent().parent().parent().attr('id') == 'qgTree') {
		//First level
		var node = e.parent().parent();
	} else {
		var node = e.parent().parent().parent().parent();
	}
	
	// определить новый класс для узла
	if (node.hasClass('qgExpandOpen')) {
		node.removeClass('qgExpandOpen').addClass('qgExpandClosed');
	} else {
		node.removeClass('qgExpandClosed').addClass('qgExpandOpen');
	}

	if (node.parent().parent().hasClass('qgExpandClosed')) {
		//var e = node.parent().parent().find('.qgExpand:first');
		expandMe(node);
	}
}