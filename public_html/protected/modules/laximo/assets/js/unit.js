function prepareImage()
{
	...
	//Ширина и высота изображения вместе с внутренними границами (padding)
	var width = img.innerWidth();
	var height = img.innerHeight();

	if (img.attr('owidth') == '') img.attr('owidth', width);
	if (img.attr('oheight') == '') img.attr('oheight', height);
...
function fitToWindow() {
	...
	jQuery('#viewport, #viewtable').css('width', Math.ceil(width*0.74));
	jQuery('#viewtable').css('width', Math.ceil(width*0.25));
	
	//console.log('viewtable - ' + Math.ceil(width*0.36));
}
