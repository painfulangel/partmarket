...
class GuayaquilDetailsList extends GuayaquilTemplate {
	var $closedimage = '/images/laximo/closed2.gif';
	var $opennedimage = '/images/laximo/openned.gif';
	var $fullreplacementimage = '/images/laximo/replacement.gif';
	var $forwardreplacementimage = '/images/laximo/replacement-forward.gif';
	var $backwardreplacementimage = '/images/laximo/replacement-backward.gif';
	var $detailinfoimage = '/images/laximo/info.gif';
	var $cartimage = '/images/laximo/cart.gif';
	var $columns = array (
		'OEM' => 3,
		'Name' => 3,
		'Price' => 3,
	...
	function Draw($catalog, $details) {
		...
		Yii::app ()->getClientScript ()->registerScriptFile($dir.'/js/jquery.tooltip.js', CClientScript::POS_END);
		Yii::app ()->getClientScript ()->registerScriptFile($dir.'/js/detailslist.js', CClientScript::POS_END);
	...
	function DrawJavaScript() {
		$html = "<script type=\"text/javascript\">\n".
				"var opennedimage = '".$this->opennedimage."'; \n".
				"var closedimage = '".$this->closedimage."'; \n".
				"jQuery(document).ready(function($){ \n".
				"jQuery('td.g_rowdatahint').tooltip({track: true, delay: 0, showURL: false, fade: 250, positionLeft: true, bodyHandler: g_getHint}); \n".
				"jQuery('img.g_addtocart').tooltip({track: true, delay: 0, showURL: false, fade: 250, bodyHandler: function() { return '".$this->GetLocalizedString("AddToCartHint")."'; } }); \n".
				"jQuery('td[name=c_toggle] img').tooltip({track: true, delay: 0, showURL: false, fade: 250, bodyHandler: function() { return '".$this->GetLocalizedString("ToggleReplacements")."'; } }); \n".
				"jQuery('img.c_rfull').tooltip({track: true, delay: 0, showURL: false, fade: 250, bodyHandler: function() { return '<h3>".$this->GetLocalizedString("ReplacementWay")."</h3>".$this->GetLocalizedString("ReplacementWayFull")."'; } }); \n".
				"jQuery('img.c_rforw').tooltip({track: true, delay: 0, showURL: false, fade: 250,	bodyHandler: function() { return '<h3>".$this->GetLocalizedString("ReplacementWay")."</h3>".$this->GetLocalizedString("ReplacementWayForward")."'; } }); \n".
				"jQuery('img.c_rbackw').tooltip({track: true, delay: 0, showURL: false, fade: 250, bodyHandler: function() { return '<h3>".$this->GetLocalizedString("ReplacementWay")."</h3>".$this->GetLocalizedString("ReplacementWayBackward")."'; } }); \n".
				"});\n".
				"</script>\n";
	...
	function DrawDetailCellValue($detail, $column, $visibility, $isArray) {
		switch ($column) {
			...
			case 'price':
				...
				return '<a href="'.$link.'" target="_blank">'.CommonExtender::FormatLocalizedString('LearnPrice').'</a>';
