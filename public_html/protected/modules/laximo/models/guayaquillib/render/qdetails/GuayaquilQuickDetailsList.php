...
class GuayaquilQuickDetailsList extends GuayaquilTemplate
{
	...
	function __construct(IGuayaquilExtender $extender)
	{
		...
		Yii::app()->getClientScript()->registerScriptFile($dir.'/js/jquery.colorbox.js', CClientScript::POS_END);
	}
		...
		/*        $note = (string)$unit['note'];
		 ...
		 if (strlen($img))
            ...
            <div class="guayaquil_unit_icons">
                <div class="guayaquil_zoom" link="'.$link.'" full="'.str_replace('%size%', 'source', $unit['imageurl']).'" title="'.$unit['code'].': '.$unit['name'].'">
                </div>
            </div>
	...
	protected function DrawEmptySet()
	{
		...
		return $this->GetLocalizedString('nothingisfounduse').'<a href="'.$link.'">'.$this->GetLocalizedString('illustratedcatalog').'</a>'.$this->GetLocalizedString('forsearchrequireddetail');
	}
}