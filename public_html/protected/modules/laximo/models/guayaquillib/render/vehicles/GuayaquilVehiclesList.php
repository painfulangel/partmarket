...
class GuayaquilVehiclesList extends GuayaquilTemplate
{
    ...
    function __construct(IGuayaquilExtender $extender)
    {
        ...
        $assetsDir = dirname(__FILE__).'/../../../../assets/js';
        Yii::app()->getClientScript()->registerScriptFile(Yii::app()->assetManager->publish($assetsDir).'/jquery.tooltip.js', CClientScript::POS_END);
        Yii::app()->getClientScript()->registerScriptFile(Yii::app()->assetManager->publish($assetsDir).'/vehicletable.js', CClientScript::POS_END);
    }
    ...
    function Draw($catalog, $vehicles)
    {
        ...
        $html = '<table class="guayaquil_table vehicles" border=1 width="100%">';
    ...
    {
        return ucfirst($column); //$this->GetLocalizedString('ColumnVehicle'.(string)$column);
    }
