 ...
 */
class WebPaymentsYandex extends CMyActiveRecord
{
    ...
    public function beforeSave()
    {
        if (parent::beforeSave()) {
            if ($this->isNewRecord) {
                $this->description = ($this->order_id) ? 'Order payment (Yandex.Money)' : 'Account replenishment (Yandex.Money)'; // Yii::t('webPayments', 'Account replenishment (Yandex.Money)');
                
                $model = new WebPayments();
                $model->value = $this->value;
                $model->comment = $this->description;
                $model->model_type = $this->type;
                $model->method = 'Yandex.Money'; // 'Яндекс.Деньги';
    ...
    {
        return md5($_POST['action'] . ';' . $_POST['orderSumAmount'] . ';' . $_POST['orderSumCurrencyPaycash'] . ';' . $_POST['orderSumBankPaycash'] . ';' . $_POST['shopId'] . ';' . $_POST['invoiceId'] . ';' . $_POST['customerNumber'] . ';' . $this->system_password);
    }
    ...
    public function rules()
    {
        ...
        return array(
            ...
            array(
                'id, start_date, finish_date, user_id, value, auth_key, total_value, order_id',
                'safe',
                'on' => 'search'
            )
        );
