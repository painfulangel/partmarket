...
<div class="container b_page_justice">
    <div class="navbar">
        <div class="nav-header">
            <div class="b_log_out pull-right">
            	<a href="<?php echo Yii::app()->createUrl('/userControl/adminUserMessages/admin'); ?>"><?php if (Yii::app()->getModule('userControl')->unreadMessages()) { ?><img src="/images/bell_yes.png" style="max-height: 20px; margin-right: 10px;"><?php } else { ?><img src="/images/bell_no.png" style="max-height: 20px; margin-right: 10px;"><?php } ?></a>
            	<?php echo CHtml::link(Yii::t('admin_layout', 'Exit'), array('/lily/user/logout')); ?>
            </div>
            <img class="logo" src="/images/admin_site/logo.png"/>
            <div class="header-title"><?php echo Yii::t('admin_layout', 'Website\'s admin control panel') ?> </div>
            <div class="b_name"><?php echo CHtml::link(Yii::app()->config->get('Site.SiteName'), array('/site/index')) ?></div>
...
<div id="myModalSubmit" class="reveal-modal">
    <h1 id="modalTitleSubmit"><?php echo Yii::t('admin_layout', 'Prepaid order') ?></h1>

    <p id="modalTextSubmit"><?php echo Yii::t('admin_layout', 'In your order, there are details which require prepayment, select prepaid option.') ?></p>

    <p id="modalCartButtonsSubmit">
        <?php echo CHtml::link(Yii::t('admin_layout', 'Full payment'), '#', array('class' => 'btn btn-primary pull-left', 'onclick' => '$(\'#prepaid_type_id\').attr(\'value\',1);$(\'#orders-form\').submit();return false;')) ?>
        <?php echo CHtml::link(Yii::t('admin_layout', 'Prepayment') . ' 30%', '#', array('class' => 'btn btn-primary pull-right', 'onclick' => '$(\'#prepaid_type_id\').attr(\'value\',0.3);$(\'#orders-form\').submit();return false;')) ?>
    </p>
    <a class="close-reveal-modal">&#215;</a>
</div>
